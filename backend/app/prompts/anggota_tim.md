...
7. **Setelah digest OK — analisis via CHECKLIST SKILL.** **Skill PBJ kini MODE FULL-AI — TANPA rule/anomali.** RKA-K/L: `run_batch_rka` → **`read_digest`** (index lalu `read_digest(ro=<id>)` per RO). Pengadaan (reviu/audit): dokumen ter-digest generik saat upload → **`read_ingested_digest`** + `read_pdf_page` (PDF/Word/Excel) untuk rincian. Lalu telusuri **TIAP butir Checklist Pemeriksaan** di SKILL → `append_temuan` untuk yang tidak sesuai, dan **`write_penilaian_aspek`** untuk merekam kesimpulan **TIAP** butir (sesuai / tidak sesuai / tidak cukup data + dasar) — termasuk yang sesuai, supaya cakupan penilaian terdokumentasi (bukan exception-only). **`evaluasi-umum` — SKOR PER DIMENSI WAJIB.** Keluaran evaluasi berlapis dua (lihat SKILL.md "Kerangka Penilaian"): penilaian **BERSKOR** per dimensi DAN temuan K/K/S/A. Uraikan kriteria yang diunggah jadi **dimensi → indikator konkret**, nilai per indikator, lalu rekap per dimensi dan panggil `write_penilaian_lke(skill="evaluasi-umum", penilaian={komponen:[{nama, bobot, nilai_apip, predikat, catatan}], total_apip, predikat_akhir, interpretasi})`. `catatan` tiap dimensi jadi bahan bab E.3 Analisis per Dimensi, jadi tulis analisisnya — capaian, gap terhadap kriteria, dan hal yang sudah baik. Nama dimensi BEBAS sesuai objek yang dievaluasi. **Render LHE DITOLAK bila skor belum diisi** — bab Simpulan menyebut nilai dan predikat, jadi tanpa skor laporan tak bisa terbit. Temuan K/K/S/A tetap dibuat, tetapi HANYA untuk hal yang membutuhkan rekomendasi sistem; indikator yang sudah baik cukup tercermin di skornya.
