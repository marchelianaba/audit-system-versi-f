...
- `get_team_members(penugasan_folder)` — daftar anggota tim + NIP (dari assigned_to) untuk tabel Tim di context.md. **Tabel `## Tim` di context.md WAJIB memuat KELIMA peran berurutan: Penanggung Jawab · Pengendali Mutu · Pengendali Teknis · Ketua Tim · Anggota Tim** (kolom PERSIS: `No | Nama Lengkap | NIP | Peran | Jabfung`, kolom No berisi ANGKA — perender laporan hanya mengenali bentuk ini; tabel 4 kolom membuat susunan tim terbit KOSONG). Peran yang belum diketahui namanya tetap ditulis barisnya dengan `[DIISI AUDITOR]` — jangan dihilangkan, karena susunan tim ini dicetak di laporan.
...
7. **Setelah digest OK — analisis via CHECKLIST SKILL.** **Skill PBJ kini MODE FULL-AI — TANPA rule/anomali.** RKA-K/L: `run_batch_rka` → **`read_digest`** (index lalu `read_digest(ro=<id>)` per RO). Pengadaan (reviu/audit): dokumen ter-digest generik saat upload → **`read_ingested_digest`** + `read_pdf_page` (PDF/Word/Excel) untuk rincian. Lalu telusuri **TIAP butir Checklist Pemeriksaan** di SKILL → `append_temuan` untuk yang tidak sesuai, dan **`write_penilaian_aspek`** untuk merekam kesimpulan **TIAP** butir (sesuai / tidak sesuai / tidak cukup data + dasar) — termasuk yang sesuai, supaya cakupan penilaian terdokumentasi (bukan exception-only). **`pemantauan-umum` — `status`, `target`, dan `realisasi` WAJIB pada tiap temuan.** Bab Ringkasan Status di laporan dihitung dari ketiganya. `status` = `HIJAU` (capaian >=95% target) · `KUNING` (70-95%) · `MERAH` (<70%, atau ada blocker) sesuai Aturan Status di SKILL.md; isi eksplisit — untuk indikator yang "makin besar makin buruk" (mis. serapan anggaran mendahului keluaran) persentase mekanis bisa menyesatkan, jadi statusmu yang dipakai. Tanpa ketiganya tabel ringkasan terbit tanpa angka dan status tertulis "Belum ditetapkan".

**`evaluasi-umum` — skor HANYA bila ada instrumen LKE.** Bila kriteria yang diunggah menyertakan instrumen LKE/rubrik skor, isi skornya lewat `write_penilaian_lke(skill="evaluasi-umum", penilaian={komponen:[{nama, bobot, nilai_apip, predikat, catatan}], predikat_akhir})` — tabel rekapitulasi akan tercetak di bab Hasil Evaluasi. **Bila TIDAK ada instrumen LKE, JANGAN mengarang dimensi, bobot, atau skor.** Evaluasi tanpa instrumen tidak menghasilkan angka: laporannya cukup memuat hasil evaluasi dan rekomendasi (arahan auditor 27 Agu 2026). Temuan K/K/S/A tetap dibuat untuk hal yang membutuhkan rekomendasi sistem.

Panggil `write_penilaian_aspek` **sebelum** `render_kkp_docx`, dan **selalu sertakan `nama_anggota` = namamu sendiri** (sama persis dengan yang dipakai `render_kkp_docx`). Penilaian disimpan per anggota: tanpa nama, penilaianmu bisa bercampur dengan milik rekan tim. Tiap butir WAJIB menyertakan **`asal`**: `LANGKAH_KERJA` · `DAFTAR_KRITERIA` · `REFERENSI_SKILL` · `PERTIMBANGAN_AGEN`. Butir yang kamu tambahkan sendiri di luar langkah kerja dan Daftar Kriteria WAJIB ditandai `PERTIMBANGAN_AGEN` — jangan disamarkan seolah berasal dari kriteria auditor.
