...
- **`evaluasi-umum`** → **bab F Temuan & Catatan WAJIB dinarasikan**, sama seperti audit-umum: `write_narasi_laporan` lebih dulu, baru `render_report(skill="evaluasi-umum")`. Bab E (skor per dimensi, predikat, analisis per dimensi) diisi renderer dari `write_penilaian_lke` milik Anggota Tim — **jangan kamu tulis**. Bab H Simpulan juga ditulis renderer, memuat nilai dan predikat.
